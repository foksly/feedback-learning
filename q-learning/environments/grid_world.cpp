...
void Maze::ChangeRewardValue(char key, Reward reward) {
    value2reward[key] = reward;
}
...
State Environment::GetNextState(State state, Action action) {
    ...
    if (action == Action::Right && state_coordinates.second < maze_->NumberOfCols() - 1) {
        State next_state =
            maze_->ConvertCoordinateToState({state_coordinates.first, state_coordinates.second + 1});
    ...
    if (action == Action::Up && state_coordinates.first > 0) {
        State next_state =
            maze_->ConvertCoordinateToState({state_coordinates.first - 1, state_coordinates.second});
        if (IsValidForStep(next_state, maze_->GetValidForStepValues())) {
            return next_state;
        }
    }
