...
class StatePreprocessor(nn.Module):
    ...
    def forward(self, coords, obses):
        ...
        field_emb = self.field_embedding(obses.view(batch_size, -1)).view(batch_size, -1)
    ...
    def to_tensor(self, state):
        ...
        obs = torch.as_tensor([state.observation], dtype=torch.long, device=device)
        return coord, obs


# class StatePreprocessor(nn.Module):
#     def __init__(self, values_range, embedding_dim, mean, std):
#         super().__init__()
#         self.mean = mean
#         self.std = std
#         self.embedding = nn.Embedding(values_range, embedding_dim)
...
#         """
#         batch_size = coords.shape[0]
#         coords_emb = self.embedding(coords).view(batch_size, -1)
#         obses = (obses.view(batch_size, -1) - self.mean) / self.std
#         return torch.cat([coords_emb, obses], dim=1)
