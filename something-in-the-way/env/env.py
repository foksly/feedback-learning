...
State = namedtuple('State', ['coordinate', 'observation'])
...
class Environment:
    ...
    def step(self, action):
        ...
        return State(self.agent_position, self._get_receptive_field(self.agent_position)), \
               reward_for_step, is_done

    def get_hints(self):
        hints = []
        for trajectory in self.hints:
            for t in trajectory:
                coordinate = Position(t[0], t[1])
                observation = self._get_receptive_field(coordinate)
                state = State(coordinate, observation)
